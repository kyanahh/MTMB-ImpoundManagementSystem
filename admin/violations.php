...
<body>
    ...
    <div class="main-container d-flex">
        ...
        <div class="content bg-light">
            ...
            <div class="px-3">
                <div class="row">
                    <div class="col-sm-2">
                        <h2 class="fs-5 mt-1 ms-2">Violations</h2>
                    </div>
                ...
                <div class="card" style="height: 600px;">
                    <div class="card-body">
                        <div class="table-responsive" style="height: 550px;">
                            <table id="violation-table" class="table table-bordered table-hover">
                                ...
                                <?php
                                    ...
                                    $result = $connection->query("SELECT * FROM violations ORDER BY violationid DESC");
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo '<button class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#editModal" onclick="loadData(' . $row['violationid'] . ')"><i class="bi bi-pencil-square"></i></button>';
                                            echo '<button class="btn btn-danger" onclick="del(' . $row['violationid'] . ')"><i class="bi bi-trash"></i></button>';
                                            echo '</div>';
                                            echo '</td>';
                                            echo '</tr>';
                                            $count++; 
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addModalLabel">Add New Violation</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="addForm">
                        <div class="mb-3">
                            <label for="violationTypeInput" class="form-label">Violation Type</label>
                            <input type="text" class="form-control" id="violationTypeInput" name="violation_type" placeholder="Enter violation type" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <label for="fineTypeInput" class="form-label">Fine Amount</label>
                            <input type="number" class="form-control" id="fineTypeInput" name="fine_amount" placeholder="Enter fine amount" required>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" id="saveViolationButton">Save</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm">
                        <div class="mb-3">
                            <div>
                                <input type="hidden" id="editId" name="violationid">
                                <label for="editType" class="form-label">Violation Type</label>
                                <input type="text" class="form-control" id="editType" name="violation_type" placeholder="Enter violation type" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="editFine" class="form-label">Fine Amount</label>
                                <input type="number" class="form-control" id="editFine" name="fine_amount" placeholder="Enter fine amount" required>
                            </div>
                        </div>
    ...
    <script>
        ...
        //---------------------------Edit Violation---------------------------//
            ...
            function loadData(violationid) {
                $.ajax({
                    url: 'violation_get.php',
                    type: 'POST',
                    ...
                    success: function (response) {
                        ...
                        if (result.success) {
                            $('#editId').val(result.data.violationid);
                            $('#editType').val(result.data.violation_type);
                            $('#editDescription').val(result.data.description);
                            $('#editFine').val(result.data.fine_amount);
                        } else {
            ...
            $('#editForm').on('submit', function (e) {
                ...
                const violationId = $('#editId').val();
                const violationType = $('#editType').val();
                const descriptionType = $('#editDescription').val();
                const fineType = $('#editFine').val();

                $.ajax({
                    url: 'violation_update.php',
                    type: 'POST',
                    data: { violationid: violationId, violation_type: violationType,
                        description: descriptionType, fine_amount: fineType 
                     },
                    success: function (response) {
                        const result = JSON.parse(response);
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            if (IdToDelete) {
                $.ajax({
                    url: 'violation_delete.php',
                    method: 'POST',
                    data: { violationid: IdToDelete },
                    dataType: 'json',
                    success: function (response) {
                        if (response.success) {
                            showDeleteToast();
        ...
        //---------------------------Search Violation Results---------------------------//c
        function search() {
            const query = document.getElementById("searchInput").value;

            // Make an AJAX request to fetch search results
            $.ajax({
                url: 'violation_search.php', 
                method: 'POST',
                data: { query: query },
                success: function(data) {
                    $('#violation-table tbody').html(data);
                },
        ...
        $(document).ready(function () {
            $('#saveViolationButton').on('click', function () {
                const violationType = $('#violationTypeInput').val();
                const description = $('#descriptionTypeInput').val();
                const fineAmount = $('#fineTypeInput').val();

                if (violationType === '' || description === '' || fineAmount === '') {
